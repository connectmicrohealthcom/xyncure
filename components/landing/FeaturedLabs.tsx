import Link from "next/link"
  ...
  type Lab = {
    ...
    logo_url: string | null; home_collection: boolean; accreditations: string[] | null
  }

  const MOCK_LABS: Lab[] = [
    { id: "1", name: "Kerala Diagnostics Centre", slug: "kerala-diagnostics", city: "Kochi", logo_url: null, home_collection: true, accreditations: ["NABL"] },
    { id: "2", name: "HealthPath Laboratory", slug: "healthpath-lab", city: "Thiruvananthapuram", logo_url: null, home_collection: true, accreditations: [] },
    { id: "3", name: "MedLabs Thrissur", slug: "medlabs-thrissur", city: "Thrissur", logo_url: null, home_collection: false, accreditations: ["NABL"] },
    { id: "4", name: "CityLab Kozhikode", slug: "citylab-kozhikode", city: "Kozhikode", logo_url: null, home_collection: true, accreditations: [] },
  ]

  export default function FeaturedLabs() {
    const [featured, ...rest] = MOCK_LABS
    return (
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <Link href={`/lab/${featured.slug}`}
                  ...
                  <div className="flex items-start justify-between mb-5">
                    <div className="w-14 h-14 rounded-full bg-gradient-to-br from-teal-400 to-teal-600 flex items-center justify-center text-white overflow-hidden shadow-sm ring-4 ring-slate-50 group-hover:ring-teal-50 transition-all duration-300">
                      {featured.logo_url ? <img src={featured.logo_url} alt={featured.name} className="w-full h-full object-cover" /> : <FlaskConical className="w-6 h-6" />}
                    </div>
                    {isNabl && (
                  ...
                  <h3 className="font-extrabold text-slate-900 group-hover:text-teal-700 transition-colors text-lg leading-snug mb-2 flex-1">{featured.name}</h3>
                  {featured.city && (
                  ...
                  return (
                    <Link key={lab.id} href={`/lab/${lab.slug}`}
                      ...
                      <div className="w-11 h-11 rounded-full bg-gradient-to-br from-teal-400 to-teal-600 flex items-center justify-center text-white overflow-hidden shadow-sm shrink-0 ring-4 ring-slate-50 group-hover:ring-teal-50 transition-all duration-300">
                        {lab.logo_url ? <img src={lab.logo_url} alt={lab.name} className="w-full h-full object-cover" /> : <FlaskConical className="w-4 h-4" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h3 className="font-extrabold text-slate-900 group-hover:text-teal-700 transition-colors text-sm truncate">{lab.name}</h3>
                        <div className="flex items-center gap-2 mt-0.5">
                          {lab.city && <span className="flex items-center gap-1 text-xs text-slate-500"><MapPin className="w-3 h-3 text-slate-400 shrink-0" />{lab.city}</span>}
                          {lab.home_collection && <span className="text-[10px] font-bold text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded-full">Home</span>}
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        {isNabl && <span className="hidden sm:flex items-center gap-1 text-[10px] font-bold text-blue-700 bg-blue-50 border border-blue-100 px-2 py-0.5 rounded-full"><BadgeCheck className="w-3 h-3" /> NABL</span>}
          ...
          <div className="text-center mt-8 sm:hidden">
            <Link href="/labs" className="inline-flex items-center gap-2 bg-slate-900 hover:bg-slate-800 text-white font-semibold px-6 py-3 rounded-full transition-colors shadow-sm">
